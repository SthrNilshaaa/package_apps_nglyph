    ...
    public static void run(String style, int brightness, Vibrator vibrator, android.content.Context context,
        ...
        if (context != null) {
            ...
            if (prefs.getBoolean("screen_off_only", false) && 
                audioStreamType != android.media.AudioManager.STREAM_RING && 
                !forceAudio) {
                ...
                if (style.startsWith("native_")) {
                    ...
                    if (effect != null) {
                        if (forceAudio && finalCtx != null) {
                            finalCtx.sendBroadcast(new android.content.Intent("org.aspends.nglyphs.ACTION_AUDIO_PLAY")
                        ...
                        try {
                            ...
                            nativeManager.setFrame(new int[15]); // Force-clear potential ghosted frames
                            if (finalCtx != null) {
    ...
    private static void executeCustomRingtone(java.io.File oggFile, int brightness, Vibrator vibrator,
        ...
        if (forceAudio) {
            try {
                ...
                context.sendBroadcast(new android.content.Intent("org.aspends.nglyphs.ACTION_AUDIO_PLAY")
                        .setPackage(context.getPackageName())
                        .putExtra("style", oggFile.getName()));

                android.util.Log.d("GlyphEffects", "Playing preview for: " + oggFile.getAbsolutePath());
                if (!oggFile.exists()) {
                    android.util.Log.e("GlyphEffects", "File does not exist: " + oggFile.getAbsolutePath());
                }
                ...
                previewPlayer.setOnErrorListener((mp, what, extra) -> {
                    android.util.Log.e("GlyphEffects", "MediaPlayer Error: what=" + what + " extra=" + extra);
                    return false;
                });
                ...
                android.util.Log.e("GlyphEffects", "MediaPlayer failure for " + oggFile.getName(), e);
            }
        } else {
                    ...
                    for (int frameIdx = 0; frameIdx < frames.length; frameIdx++) {
                        ...
                        if (sleepTime > 0) {
                            ...
                            if (frameIdx + skipCount < frames.length) {
                                frameIdx += skipCount;
                                // Log.w("GlyphEffects", "Sync Lag: skipping " + skipCount + " frames");
                            }
                        }
                    }
