...
public class AnimationManager {
    ...
    private static synchronized boolean requestLock(int priority) {
        if (appContext != null) {
            android.content.SharedPreferences prefs = appContext.getSharedPreferences(appContext.getString(R.string.pref_file), android.content.Context.MODE_PRIVATE);
            if (prefs.getBoolean("is_light_on", false)) return false;
            
            // Block all non-critical animations if user is on an active call and setting is enabled
        ...
        if (priority >= activePriority || (activePriority == PRIORITY_PROGRESS && targetLevel < 0.01f)) {
    ...
    public static void setBackgroundGlyph(GlyphManagerV2.Glyph glyph, int brightness) {
        ...
        if (appContext != null) {
            android.app.KeyguardManager km = (android.app.KeyguardManager) appContext.getSystemService(android.content.Context.KEYGUARD_SERVICE);
            android.os.PowerManager pm = (android.os.PowerManager) appContext.getSystemService(android.content.Context.POWER_SERVICE);
    ...
    private static void updateMeter(float level, int maxZones) {
        // Guard: If a high-priority effect (like a notification or flip pattern) is running,
        // we block regular meter updates. BUT we allow a final "clear" (level 0) if the
        // animation loop has been interrupted (isAnimationRunning = false).
        if (activePriority == PRIORITY_HIGH && level < 0.01f && isAnimationRunning) return;
    ...
    public static void showAssistantLevel(int percentage, android.content.Context context) {
        if (context != null) {
            android.content.SharedPreferences prefs = context.getSharedPreferences(context.getString(R.string.pref_file),
                    android.content.Context.MODE_PRIVATE);
            if (prefs.getBoolean("is_light_on", false))
            ...
            try {
                while (isAnimationRunning && activePriority == priority && runningLoopPriority == priority) {
                    ...
                    if (forcedFirstFrame || Math.abs(diff) > 0.01f) {
                        ...
                        // Hold state: level is non-zero and reached. 
                        // Update once to ensure precision then sleep longer to save CPU.
                        updateMeter(targetLevel, maxZones);
                        SystemClock.sleep(200); // Longer sleep during hold
                        continue; 
                    }
                ...
                updateMeter(0f, maxZones);
                // Transition: Specifically clear the hardware frame path. This ensures that
                // if an animation was interrupted by a high-priority effect, the "frame"
                // data doesn't "ghost" on the hardware after the effect finishes.
