...
public class StyleAdapter extends RecyclerView.Adapter<StyleAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.card.setCardBackgroundColor(isSelected ? (primaryColor & 0x22FFFFFF) : surfaceVariant);
        ...
        holder.itemView.setOnClickListener(v -> {
            ...
            if (isCustomStyle) {
                GlyphEffects.run(value, prefsProvider.getBrightness(), vibrator, context, audioStreamType, true);
            } else if ("native_flip".equals(value)) {
                GlyphEffects.run("stock", prefsProvider.getBrightness(), vibrator, context, audioStreamType, true);
