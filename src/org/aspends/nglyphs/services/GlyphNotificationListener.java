...
public class GlyphNotificationListener extends NotificationListenerService implements SensorEventListener {
        ...
        unlockReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (Intent.ACTION_USER_PRESENT.equals(intent.getAction())) {
                    lastMovementTime = SystemClock.elapsedRealtime(); // Reset idle timer for battery saver
    ...
    public void onNotificationPosted(StatusBarNotification sbn) {
        ...
        boolean isDeviceUnlocked = pm != null && pm.isInteractive() && km != null && !km.isKeyguardLocked();
        ...
        if (prefs.getBoolean("master_allow", false)) {
            ...
            if (prefs.getBoolean("screen_off_only", false)) {
                ...
                if (pm2 != null && pm2.isInteractive()) return;
            }
    ...
    private void performEssentialLightUpdate() {
        ...
        // Immediate Suppression: If device is active and unlocked, no essential light can fire
        android.app.KeyguardManager kmCheck = (android.app.KeyguardManager) getSystemService(Context.KEYGUARD_SERVICE);
        android.os.PowerManager pmCheck = (android.os.PowerManager) getSystemService(Context.POWER_SERVICE);
        boolean isDeviceActiveAndUnlocked = pmCheck != null && pmCheck.isInteractive() && kmCheck != null && !kmCheck.isKeyguardLocked();
        
        android.util.Log.d("GlyphNotification", "performEssentialLightUpdate: activeAndUnlocked=" + isDeviceActiveAndUnlocked);
        ...
        // Block all Essential lights if user is on an active call and setting is enabled
        ...
        for (GlyphManagerV2.Glyph glyph : GlyphManagerV2.Glyph.values()) {
            boolean shouldShow = !isDeviceActiveAndUnlocked && masterAllow && hasActiveEssential && activeGlyphs.contains(glyph) && !isBatterySaverActive;
            int val = shouldShow ? brightness : 0;
            AnimationManager.setBackgroundGlyph(glyph, val);
        }
        
        android.util.Log.d("GlyphNotification", "Essentials Updated: hasActive=" + hasActiveEssential + " (master=" + masterAllow + ")");
    }
    ...
    private void performIgnoreLogic() {
        ...
        // Aggressively refresh local map from system state to avoid ghosts/stale notifications
        try {
            StatusBarNotification[] active = getActiveNotifications();
            localActiveEssentials.clear();
            if (active != null) {
