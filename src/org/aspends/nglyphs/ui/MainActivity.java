...
public class MainActivity extends AppCompatActivity {
    ...
    private MaterialSwitch switchMaster, switchFlip, switchLockscreenOnly, switchSleepMode,
            ...
            switchBluetoothBatteryCombine, switchNotifCooldown, switchStopDuringCall, switchTorch;
                            ...
                            if (uri != null) {
                                ...
                                if (!isCsv && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M && !android.provider.Settings.System.canWrite(this)) {
                                    new com.google.android.material.dialog.MaterialAlertDialogBuilder(this)
                                        ...
                                        .setMessage("To set this audio as a ringtone, Glyph Manager needs permission to modify system settings. Grant it now?")
    ...
    private void showRootErrorDialog() {
        new com.google.android.material.dialog.MaterialAlertDialogBuilder(this)
                ...
                .setMessage("Glyph Manager requires root access to directly control the LED hardware nodes. You can still browse settings, but many features will not function without root permissions granted in Magisk/Kitsune Mask.")
                .setPositiveButton("I Understand", (d, w) -> d.dismiss())
                .setNeutralButton("Retry", (d, w) -> checkRootAccess())
                .show();
    }
    ...
    private void setupListeners() {
        ...
        switchMaster.setOnCheckedChangeListener((v, isChecked) -> {
            ...
            if (isChecked) {
                ...
                // ShakeToGlyphService remains running if enabled, for Torch independence
        ...
        try {
            ...
            if (powerManager != null && !powerManager.isIgnoringBatteryOptimizations(getPackageName())) {
                new MaterialAlertDialogBuilder(this)
                        ...
                        .setMessage(
                                "To ensure Shake to Glyph and Assistant Visualizer run reliably in the background, please disable battery optimization for Glyph Manager.")
            ...
            Log.e("MainActivity", "Failed to check battery optimization status", e);
        }
    }
    ...
    private void refreshUIState() {
        ...
        for (MaterialCardView c : cards) {
            ...
            if (c == cardSleepTime || c == cardBrightness || c == cardTorchBrightness) cardEnabled = isMasterAllowed;
            ...
            if (c != cardBrightness && c != cardTorchBrightness) {
                ...
                if (!cardEnabled) {
                    c.setCardBackgroundColor(com.google.android.material.color.MaterialColors.getColor(c, com.google.android.material.R.attr.colorSurfaceVariant));
                } else {
                    c.setCardBackgroundColor(com.google.android.material.color.MaterialColors.getColor(c, com.google.android.material.R.attr.colorSurfaceContainer));
                }
            }
        }
        ...
        MaterialSwitch[] switches = { switchFlip, switchLockscreenOnly, switchBattery, switchShake,
                ...
                switchBatteryWhileOn, switchNotifCooldown, switchStopDuringCall };
        ...
        for (MaterialSwitch s : switches) {
            if (s != null) {
                ...
                if (s == switchVolumeFlipOnly) parentEnabled &= switchVolumeBar.isChecked();
                if (s == switchShakeWhileOn) parentEnabled &= switchShake.isChecked();
                if (s == switchGlyphProgressFlippedOnly) parentEnabled &= switchGlyphProgress.isChecked();
                if (s == switchBluetoothBatteryCombine) parentEnabled &= switchBluetoothBattery.isChecked();
                if (s == switchBatteryWhileOn) parentEnabled &= switchBattery.isChecked();
        ...
        Slider[] sliders = { slider, sliderShakeSensitivity, sliderRingNotifHapticStrength, sliderTorch, sliderProgressBrightness };

        for (Slider sl : sliders) {
            if (sl != null) {
                boolean parentEnabled = generalEnabled;
                if (sl == sliderShakeSensitivity) parentEnabled &= switchShake.isChecked();
                if (sl == sliderProgressBrightness) parentEnabled &= switchGlyphProgress.isChecked();
